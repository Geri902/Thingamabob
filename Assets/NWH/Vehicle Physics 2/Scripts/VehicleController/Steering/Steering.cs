...
namespace NWH.VehiclePhysics2
{
    ...
    public partial class Steering : VehicleComponent
    {
        ...
        public virtual void CalculateSteerAngles()
        {
            ...
            if (!useRawInput && !returnToCenter && horizontalInput > -Vehicle.INPUT_DEADZONE && horizontalInput < Vehicle.INPUT_DEADZONE)
            {
                return;
            }
            ...
            foreach (WheelGroup wheelGroup in vehicleController.powertrain.wheelGroups)
            {
                ...
                if (wheelGroup.Wheels.Count == 2 && vehicleController.wheelbase > Vehicle.SMALL_NUMBER && wheelGroup.addAckerman)
                {
        ...
        public float CalculateWheelbase()
        {
            // Calculate wheelbase
            float wheelbase = -1;
            int wheelCount = vehicleController.powertrain.wheelCount;
            if (wheelCount == 4)
            {
                wheelbase = Vector3.Distance(
                    vehicleController.powertrain.wheels[0].wheelUAPI.transform.position,
                    vehicleController.powertrain.wheels[2].wheelUAPI.transform.position);
            }
            else if (wheelCount == 2)
            {
                wheelbase = Vector3.Distance(
                    vehicleController.powertrain.wheels[0].wheelUAPI.transform.position,
                    vehicleController.powertrain.wheels[1].wheelUAPI.transform.position);
            }
            return wheelbase;
        }
